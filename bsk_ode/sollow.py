...
def rolling_online_predict_econ_nonbranched(state, retrain_every=5, max_steps=None):
    ...
    # Ensure F_star is a tensor on the same device/dtype
    F_star = state["F_star"]
    if not isinstance(F_star, torch.Tensor):
        F_star = torch.tensor(F_star, dtype=dtype, device=device)
    else:
        F_star = F_star.to(dtype=dtype, device=device)
    ...
    # Make all state tensors live on the same device/dtype
    alphas = torch.zeros(end_idx + 1, dtype=dtype, device=device)
    alphas[:n0 + 1] = state["alpha0"].to(dtype=dtype, device=device)

    K_prev = state["K_prev"].clone().to(dtype=dtype, device=device)
    I1     = state["I1"].clone().to(dtype=dtype, device=device)

    F_pred = torch.zeros(end_idx + 1, dtype=dtype, device=device)
    y_pred = torch.zeros(end_idx + 1, dtype=dtype, device=device)

    F_pred[:n0 + 1] = state["F_pred_train"].to(dtype=dtype, device=device)
    y_pred[:n0 + 1] = state["y_pred_train"].to(dtype=dtype, device=device)
    ...
    for i in range(n0 + 1, end_idx + 1):
        ...
        I1 = torch.cat([
            I1,
            torch.tensor([I1_i.item()], dtype=dtype, device=device)
        ])
